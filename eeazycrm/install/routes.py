...
def sys_info():
    ...
    return render_template("install/sys_info.html", title="System Information",
                           system_info=system_info, py_ver=sys.version, env_vars=env_vars)
...
def finish():
    ...
    if request.method == 'POST' and form.validate():
        ...
        sql_text = text(SAMPLE_DATA)

        if form.import_sample_data.data:
            db.session.execute(sql_text.params(
                admin_first_name=session['admin_first_name'],
                admin_last_name=session['admin_last_name'],
